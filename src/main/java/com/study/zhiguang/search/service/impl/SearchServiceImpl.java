package com.study.zhiguang.search.service.impl;
...
import com.study.zhiguang.search.api.dto.SearchResponse;
import com.study.zhiguang.search.api.dto.SuggestResponse;
import com.study.zhiguang.search.service.SearchService;
...
public class SearchServiceImpl implements SearchService {
     ...
     * size 返回多少条
     * tagsCSV 标签过滤条件
     * after 游标分页参数 base64url便阿门
     * currentUserIdNullable 当前用户id
     * 游标的话，只有比游标小的（或者大的）才需要进入排序窗口
     * 而offset基本是全量扫描
     * 所以深分页的情况下游标更加好
     */
                        ...
                        .query(qb -> qb.functionScore(fs -> fs
                                .query(qb2 -> qb2.bool(bq -> {
                                    bq.must(m -> m.multiMatch(mm -> mm.query(q)
                                            .fields("title^3", "body"))); //用户输入的 q 去做 multi_match 搜索字段是 title 和 body 其中 title^3 表示标题字段权重是正文的 3 倍
                                    bq.filter(f -> f.term(t -> t.field("status")
                                            .value(v -> v.stringValue("published")))); //必须是公开发布的

                                    if (tags != null && !tags.isEmpty()) {
                                        bq.filter(f -> f.terms(t -> t.field("tags")
                                                .terms(tv -> tv.value(tags.stream().map(FieldValue::of).toList())))); //如果有tags标签，文档的tags标签命中这些标签
                                    }
                                    return bq;
                                }))
                                // 对点赞数收藏数设置权重
