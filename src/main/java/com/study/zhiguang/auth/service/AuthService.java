package com.study.zhiguang.auth.service;


import com.study.zhiguang.auth.api.dto.*;
import com.study.zhiguang.auth.audit.LoginLogService;
import com.study.zhiguang.auth.config.AuthProperties;
import com.study.zhiguang.auth.model.ClientInfo;
import com.study.zhiguang.auth.model.IdentifierType;
import com.study.zhiguang.auth.token.JwtService;
import com.study.zhiguang.auth.token.RefreshTokenStore;
import com.study.zhiguang.auth.token.TokenPair;
import com.study.zhiguang.auth.util.IdentifierValidator;
import com.study.zhiguang.auth.verification.*;
import com.study.zhiguang.common.exception.BusinessException;
import com.study.zhiguang.common.exception.ErrorCode;
...
public class AuthService {
    ...
    public SendCodeResponse sendCode(SendCodeRequest request) {
        validateIdentifier(request.identifierType(), request.identifier()); //验证邮箱或者手机号格式
        String normalized = normalizeIdentifier(request.identifierType(), request.identifier());
        boolean exists = identifierExists(request.identifierType(), normalized); //查看用户是否已经存在
        if (request.scene() == VerificationScene.REGISTER && exists) { //已经被注册过
        ...
        if ((request.scene() == VerificationScene.LOGIN || request.scene() == VerificationScene.RESET_PASSWORD) && !exists) {
            throw new BusinessException(ErrorCode.IDENTIFIER_NOT_FOUND); //没这个账号
        }
