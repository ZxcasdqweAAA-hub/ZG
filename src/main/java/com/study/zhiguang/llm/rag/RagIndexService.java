package com.study.zhiguang.llm.rag;
...
import com.study.zhiguang.config.EsProperties;
...
public class RagIndexService {
    ...
    public int reindexSinglePost(long postId) {
        ...
        // 指纹检测：如未变化过则跳过重建
        String currentSha = row.getContentUrl();
    ...
    private List<String> chunkMarkdown(String text) {
        ...
        return getChunk(paras);
    }

    private static List<String> getChunk(List<String> paras) {
        ...
        for (String para : paras) {
            if (para.length() <= 800) {
                ...
                while (start < para.length()) {
                    int end = Math.min(para.length(), start + 800);
